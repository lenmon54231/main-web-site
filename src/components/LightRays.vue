...
function getAnchorAndDir(origin: RaysOrigin, w: number, h: number): AnchorAndDirection {
  const outside = 0.2
  switch (origin) {
    case 'top-left':
...
<template>
  <div ref="containerRef" class="pointer-events-none relative z-[3] h-full w-full overflow-hidden" :class="[className]" />
</template>
